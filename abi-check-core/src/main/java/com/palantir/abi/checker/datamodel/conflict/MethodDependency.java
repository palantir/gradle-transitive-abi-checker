...
public interface MethodDependency extends Dependency {
    ...
    default ClassTypeDescriptor targetClass() {
        return targetMethod().clazz();
    }

    MethodReference targetMethod();

    static MethodDependency of(
            DeclaredMethod method, CallSite<MethodReference> calledMethod, List<ClassTypeDescriptor> reachabilityPath) {
        return ImmutableMethodDependency.builder()
                .reachabilityPath(reachabilityPath)
                .fromMethod(method.reference())
                .fromLineNumber(calledMethod.lineNumber())
                .targetMethod(calledMethod.reference())
                .build();
    }
}
